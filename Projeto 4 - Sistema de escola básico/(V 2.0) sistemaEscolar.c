...
void menuAlunos(){
	...
	printf("5 - Pesquisar estudantes pelo nome\n");
		...
		case 1:{
			...
			switch(verificarID(idLido)){
				...
				case 1:{
					...
					do {
						fscanf(listaAlunos, "%d|%60[^|]|%d/%d/%d|%d", &aluno.id, aluno.nome, &aluno.diaNasc, &aluno.mesNasc, &aluno.anoNasc, &aluno.idade);
...
void pesquisarAluno(){
	...
	printf("PESQUISA DE ESTUDANTES PELO NOME\n\n");
	...
	while(fscanf(listaAlunos, "%d|%60[^|]|%d/%d/%d|%d", &aluno.id, aluno.nome, &aluno.diaNasc, &aluno.mesNasc, &aluno.anoNasc, &aluno.idade) == 6){	
		char *ocorrencia = strstr(aluno.nome, nomeProcurado);	
	...
	if (!encontrou){
		printf("Estudante nao encontado!\n\n");
	}
...
void listarNota(){
	...
	switch(verificarID(idProcurado)){
		...
		case 1:{
			...
			do {
				fscanf(listaAlunos, "%d|%60[^|]|%*d/%*d/%*d|%*d", &aluno.id, aluno.nome);
